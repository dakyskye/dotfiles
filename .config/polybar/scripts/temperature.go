package main

import (
	"context"
	"fmt"
	"log"
	"net/http"
	"os/exec"
	"regexp"
	"time"

	"golang.org/x/sync/errgroup"
)

type temperature struct {
	current   string
	requested time.Time
}

func (t *temperature) listener(w http.ResponseWriter, r *http.Request) {
	if r.Method != "POST" {
		return
	}

	task := r.FormValue("task")

	if task == "next" {
		if t.current == "gpu" {
			fmt.Println(t.cpu())
		} else if t.current == "cpu" {
			fmt.Println(t.hdd())
		} else if t.current == "hdd" {
			fmt.Println(t.gpu())
		}
	} else if task == "current" {
		t.cur()
	}
}

func (t *temperature) cur() {
	if t.current == "gpu" {
		fmt.Println(t.gpu())
		t.requested = time.Now()
	} else if t.current == "cpu" {
		fmt.Println(t.cpu())
		t.requested = time.Now()
	} else if t.current == "hdd" {
		fmt.Println(t.hdd())
		t.requested = time.Now()
	}
}

func (t *temperature) gpu() string {
	out, err := exec.Command("sensors").Output()
	if err != nil {
		return err.Error()
	}
	pattern := regexp.MustCompile(`(?m)nouveau-pci-0600\n.*\n.*\ntemp1: +\+(?P<temp>\d+)\.`)
	t.current = "gpu"
	return "G +" + pattern.FindStringSubmatch(string(out))[1]
}

func (t *temperature) cpu() string {
	out, err := exec.Command("sensors").Output()
	if err != nil {
		return err.Error()
	}
	pattern := regexp.MustCompile(`k10temp-pci-00c3(\n.*)+Tdie:( +)\+(?P<temp>\d+)\.`)
	t.current = "cpu"
	return "C +" + pattern.FindStringSubmatch(string(out))[1]
}

func (t *temperature) hdd() string {
	out, err := exec.Command("nc", "localhost", "7634").Output()
	if err != nil {
		return err.Error()
	}
	pattern := regexp.MustCompile(`(?m)\|SAMSUNG.*\|(?P<temp>\d+)\|`)
	t.current = "hdd"
	return "H +" + pattern.FindStringSubmatch(string(out))[1]
}

func main() {
	temp := temperature{current: "gpu"}
	temp.cur()
	temp.requested = time.Now()

	http.HandleFunc("/", temp.listener)

	tasks, ctx := errgroup.WithContext(context.Background())

	tasks.Go(func() error {
		return http.ListenAndServe(":9096", nil)
	})

	tasks.Go(func() error {
		tick := time.NewTicker(time.Second * 30)
		defer tick.Stop()
		for {
			select {
			case <-tick.C:
				since := time.Since(temp.requested)
				if since.Seconds() < 30 {
					<-time.NewTimer(30000 - since).C
					temp.cur()
				} else {
					temp.cur()
				}
			case <-ctx.Done():
				return nil
			}
		}
	})

	if err := tasks.Wait(); err != nil {
		log.Fatal("An error occurred:", err)
	}
}
